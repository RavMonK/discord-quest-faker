...
ถ้าอยากเลือกตัวอื่น กดลูกศร **▸** หน้าชื่อเกมเพื่อกางรายการ แล้ว Start/Stop ทีละตัวได้
...
**ตัวเลือกทดลอง — ข้ามแพลตฟอร์ม:** ติ๊ก **All platforms** ในหน้าเว็บ (หรือ `--exe-os all` ใน CLI)
จะเห็น executable ของทุกระบบ พร้อมป้ายบอกว่าเป็นของระบบไหน แล้วสั่งรันได้
บน Unix นามสกุลไฟล์ไม่มีความหมาย ระบบจึงสร้างและรันไฟล์ชื่อ `redsteam.exe` บน Mac ได้จริง

> ⚠️ **ยังไม่ได้ยืนยันว่าใช้ได้** — entry ในลิสต์มี field `os` กำกับ Discord ฝั่ง Mac
> น่าจะกรองมาเทียบเฉพาะ `darwin` ทำให้โปรเซสชื่อ `.exe` ไม่ถูกนับ แต่ผมไม่มีเครื่อง Mac
> ให้ทดสอบ ตัวเลือกนี้มีไว้ให้คุณลองเองแล้วดูผลจริง
...
วิธีนี้ก็จะไม่ทำงาน — เป็นอีกจุดที่ยังไม่ได้ทดสอบ
...
| `port 5011 is already in use` | เปลี่ยน `port` ใน `config.json` หรือรันด้วย `--port 8080` |
